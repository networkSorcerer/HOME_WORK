...
public class MemberLoginInfoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * response.getWriter();
		 * 
		 * HttpSession session = request.getSession(false); MemberDTO dto =(MemberDTO)
		 ...
		 * +"</td>"); out.println("</tr>"); out.println("<td>이메일</td>");
		 * out.println("<td>" + dto.getMemberEmail()+"</td>"); out.println("</tr>");
		 * out.println("</table>"); out.println("</div>");
		 * 
		 * out.println("<div>"); out.println("<a href='/servlet/logout'>로그아웃</a>");
		 * out.println("</div>"); } else {
		 * out.print("<div>로그인 상태가 아닙니다.<br/>로그인부터 다시 진행해 주세요 </div>");
		 * out.print("<div><a href='/servlet/jdbc/login.jsp'>로그인 화면으로 이동하기</a><div>");
		 * 
