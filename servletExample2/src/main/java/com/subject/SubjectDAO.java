...
public class SubjectDAO {
	...
	return list;
		 ...
		 * 
		 * try { con = getConnection(); pstmt = con.prepareStatement(sql.toString());
		 * rs= pstmt.executeQuery();
		 * 
		 * while(rs.next()) { svo = new SubjectVO(); svo.setNo(rs.getInt("no"));
		 * svo.setS_num(rs.getString("s_num")); svo.setS_name(rs.getString("s_name"));
		 * 
		 * list.add(svo); } }catch (SQLException se) {
		 * System.out.println("조회에 문제가 있어 잠시후에 다시 진행해 주세요"); se.printStackTrace();
		 * }catch (Exception e) { System.out.println("error = [" + e + "]" ); }finally {
		 * close(rs); close(pstmt); close(con); } return list;
		 */
	}
